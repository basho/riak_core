...
%%
%% This is a facade over riak_core_job_manager, and ALL new code should be
...
-type jobrec()      ::  #jobrec{}.
-type manager()     ::  atom() | pid(). % riak_core_job_manager process
...
-spec work_setup(
    Manager :: manager(), Context :: ctx(), WArgs :: term(), WProps :: term())
        -> ctx().
%
% riak_core_job:job.work.setup callback
%
work_setup(Mgr, #ctx{vnode = VNode, wmod = WMod} = Context, WArgs, WProps) ->
    {'ok', WState} = WMod:init_worker(VNode, WArgs, WProps),
    Context#ctx{manager = Mgr, state = WState}.
...
%
work_main(#ctx{wmod = WMod, state = WStateIn} = Context, Work, Origin) ->
...
terminate(_Why, State) ->
    ...
    [riak_core_job_manager:cancel(Id, 'true')
