...
reassign_indices(CState) ->
    reassign_indices(CState, [], time_compat:timestamp(), fun no_log/2).
...
init([]) ->
    schedule_tick(),
    {ok, #state{changes=[], seed=time_compat:timestamp()}}.
...
clear_staged(State) ->
    ...
    State#state{changes=[], seed=time_compat:timestamp()}.
...
do_maybe_force_ring_update(Ring) ->
    case compute_next_ring([], time_compat:timestamp(), Ring) of
        {ok, NextRing} ->
...
do_claimant(Node, CState, Log) ->
    do_claimant(Node, CState, [], time_compat:timestamp(), Log).
