    ...
    {foreach,
     ...
     {timeout, 60, { "group grant/revoke on type/bucket works",
                     fun() ->
                             ...
                             ?assertMatch({false, _, _}, riak_core_security:check_permissions({"riak_kv.get", {<<"default">>, <<"mybucket">>}}, Ctx)),
                             ?assertEqual(ok, riak_core_security:alter_user(<<"user">>, [{"groups", ["group"]}])),
                             %% make sure we would print the appropriate data, and only the appropriate data
                             %% TODO Perhaps we could assert here that we don't print the password for a user?
                             ...
                             ?assertMatch([{table, [[{username, <<"user">>}, {'member of', "group"},
                                          ...
                                          riak_core_security:format_user(<<"user">>)),
                             ?assertEqual(ok, riak_core_security:print_sources()),
                             ?assertMatch([{table, [[{group, <<"group">>}, {'member of', []},
                                                    {options, _}]]}],
                                          riak_core_security:format_group(<<"group">>)),
                             ?assertMatch([{table, [[{group, <<"group">>}, {'member of', []},
                                                     {options, _}]]}],
                                          riak_core_security:format_group(<<"group">>)),
                             ok
     ...
     {timeout, 60, { "all grant/revoke on type/bucket works",
                     fun() ->
                             ...
                             ?assertMatch([{table, [[{username, <<"user">>}, {'member of', []},
                                          ...
                                          riak_core_security:format_user(<<"user">>)),
                             ?assertEqual(ok, riak_core_security:print_sources()),
                             ?assertEqual([{table, []}], riak_core_security:format_groups()),
                             ?assertEqual({error, {unknown_group, <<"all">>}}, riak_core_security:format_group(<<"all">>)),
                             ok
                     end}},
     {timeout, 60, { "groups can be members of groups and inherit permissions from them",
                     fun() ->
                             ...
                             ?assertMatch([{table, [
                                                    [{group, <<"sysadmin">>}, {'member of', []}, {options, _}],
                                                    [{group, <<"superuser">>}, {'member of', "sysadmin"}, {options, _}]]}],
                                          riak_core_security:format_groups()),
                             ?assertMatch([{table, [[{group, <<"superuser">>}, {'member of', "sysadmin"}, {options, _}]]}],
                                          riak_core_security:format_group(<<"superuser">>)),
                             ?assertMatch([{table, [[{group, <<"sysadmin">>}, {'member of', []}, {options, _}]]}],
                                          riak_core_security:format_group(<<"sysadmin">>)),
     ...
     {timeout, 60, { "Expected user/group errors",
                     fun() ->
                             ...
                             ?assertMatch({error, _}, riak_core_security:format_group(<<"sysadmin">>)),
                             ?assertEqual(ok, riak_core_security:add_group(<<"sysadmin">>, [])),
